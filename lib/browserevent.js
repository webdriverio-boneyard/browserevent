...
var BrowserEvent = module.exports.init = function(webdriverInstance) {
    ...
    /**
     * overwrite session command to be able to close the socket connection
     */
    var session = webdriverInstance.session;
    webdriverInstance.addCommand('session', function(doWhat) {

        if(doWhat.toUpperCase() === 'DELETE') {
            that.socket.disconnect('unauthorized');
            that.app.close();
        }

        session.apply(webdriverInstance, arguments);
